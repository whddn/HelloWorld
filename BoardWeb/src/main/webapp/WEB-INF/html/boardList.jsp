	...
	<div class="row">
		<div class="col-sm-4">
			<select name="searchCondition" class="form-control">
				...
				<option value="W" ${searchCondition ne null && searchCondtion == 'W' ? 'selected' : '' }>작성자</option>
				<option value="TW" ${not empty searchCondition and searchCondition eq 'TW' ? 'selected' : ''}>제목 & 작성자</option>
			</select>
		</div>
		<div class="col-sm-6">
...
<table class="table">
	...
	<tbody>
		<c:forEach var="bvo" items="${list }">
			<tr>
				<td align="center"> 
					<a href="board.do?page=${paging.currentPage }&keyword=${empty keyword ? '' : keyword}&searchCondition=${empty searchCondition ? '':searchCondtion }&board_no=${bvo.boardNo} ">${bvo.boardNo }</a>
				</td>
	...
	<ul class="pagination">
		...
		<c:choose>
			<c:when test="${paging.next }">
				<li class="page-item">
					<a class="page-link" href="boardList.do?keyword=${keyword eq null ? '' : keyword }&searchCondition=${searchCondition eq null ? '' : searchCondition }&page=${paging.endPage + 1}">Next</a>
				</li>
			</c:when>
			<c:otherwise>
				<li class="page-item disabled">
